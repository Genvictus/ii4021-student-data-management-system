...
async function getHodPublicKey(departmentId: string): Promise<RsaPublicKey> {
    const response = await api.get("api/v1/users", { params: { departmentId: departmentId, role: "HOD" } });
...
async function encryptDataAndKeys(transcript: TranscriptWithStudent, encryptionKey: string): Promise<TranscriptUpdatePayload> {
    ...
    payload.encryptedTranscriptData = encryptTranscriptEntries(transcript.transcriptData!, aesKey);
...
export async function createStudentTranscript(transcript: TranscriptWithStudent, encryptionKey: string): Promise<ResponseFormat<string[] | null>> {
    try {
